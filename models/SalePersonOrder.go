...
type SalePersonOrder struct {
	Id           int64
	OrderNo      string //订单号码
	SalePersonId int64  //用户ID
	ProductId    int64  //用户类型ID
	ProductName  string `orm:"size(1000)"`

	SalePersonName         string `orm:"size(150)"`
	SalePersonMobile       string `orm:"size(150)"`
	SalePersonEmail        string `orm:"size(500)"`
	SalePersonWeixin       string `orm:"size(500)"`
	SalePersonWeixinOpenId string `orm:"size(500)"`
	...
	SalePersonType   int64     // 0 普通用户 1 超级用户（终身用户）
...
func (m *SalePersonOrder) CreateOrder() (int64, bool) {
	var (
		oid int64
	)
...
//生成订单号
func (m *SalePersonOrder) GetRandOrderNo() string {
...
func (m *SalePersonOrder) GetSalePersonOrderByOrderNo(orderNo string) bool {
	o := orm.NewOrm()
...
///最新用户购买列表
func (m *SalePersonOrder) GetLastSalePersonList(pagesize int64) []*SalePersonOrder {
	var info SalePersonOrder
	list := make([]*SalePersonOrder, 0)

	info.Query().OrderBy("-id").Limit(pagesize, 0).All(&list, "Id", "SalePersonName", "SalePersonMobile", "SalePersonType", "AddTime")

	return list
}

//获取等待发货列表
func (m *SalePersonOrder) GetWaitForSendList(pagesize int64) []*SalePersonOrder {
	var info SalePersonOrder
	list := make([]*SalePersonOrder, 0)
	query := info.Query()
	query = query.Filter("status", 1)
	query = query.Filter("is_send", 0).OrderBy("Id")
	query.Limit(pagesize, 0).All(&list, "Id", "ProductId", "PayAmount", "OrderNo", "SalePersonEmail", "SalePersonId", "SalePersonMobile")
	return list
}

func (m *SalePersonOrder) Insert() error {
...
func (m *SalePersonOrder) Update(fields ...string) error {
...
func (m *SalePersonOrder) Delete() error {
...
func (m *SalePersonOrder) Query() orm.QuerySeter {
